...
Ожидаемо: `587 examples, 0 failures`, `124 files inspected, no offenses detected`, пять словарей `ok`.
...
**6:00–6:30 — качество.**
`bundle exec rake check` — 587 тестов, линтер, валидация словарей по JSON-схемам. Каждый анализатор
...
| 1A | Парсер и анализаторы, CLI с разбором | **готово, 587 тестов** |
