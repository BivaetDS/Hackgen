...
| 3 | Первый аргумент `failure` | Только символы из эталона (`:unauthorized`, `:too_many_requests`, `:unprocessable_entity`) плюс `:provider_error` для остального; второй аргумент — i18n-ключ `provider.<canonical>` | `canonical_contract.yml → error_codes` |
...
| 15 | Порог уверенности | ≥ 0.75 — код без TODO; 0.4–0.75 — код + `TODO(confidence …)` + warning; < 0.4 — заглушка `NotImplementedError` + запись в отчёт | `Generator::TemplateData` (константы порогов), `operations.yml → thresholds` |
