...
1. MockServerGenerator + mock_server.rb.erb: Sinatra-мок провайдера из спеки (маршруты, валидация тел json_schemer, ответы из examples по кодам, генерация вебхука с подписью по конвенции). Rake-задача e2e: поднять мок на случайном порту, прогнать сгенерированный сервис с base_contract.rb против него, проверить статусы. Мок — только dev/демо, не часть сгенерированного контракта.
2. RegenerationDiff и флаг --diff: сравнение существующего output с новым, обнаружение ручных правок (W601), вывод unified diff, без перезаписи.
3. S4 cardpay.yaml (OpenAPI 3.1, type как массив, несколько create-операций) и S5 legacy_swagger2.yaml (Swagger 2.0 → конвертер в 3.0 внутри парсера, W103; нет operationId; нет examples; form-urlencoded). Golden для обеих.
4. S6: возьми реальную публичную OpenAPI-спеку платёжного API (лицензионно чистую, укажи источник в файле), урежь до 5–8 операций, проведи через конвейер, зафиксируй golden и список warnings. Это самый убедительный тест для жюри.

Критерий: e2e против мока зелёный; S4–S6 в регрессии; --diff показывает ручную правку в сгенерированном файле.
```

---

## Промпт 6 — финальное ревью перед защитой

```
Проведи ревью проекта глазами технического жюри по таблице «Технические жюри» в docs/TZ.md. Для каждого подпункта (всего 27) напиши: где в коде/выходе это доказывается, как показать за 20 секунд на демо, и что бы ты снял, будь ты строгим жюри. Отдельно проверь: 1) нет ни одного вызова нейросети/внешнего API в рантайме; 2) доля Ruby по cloc; 3) все зависимости open-source; 4) rake check зелёный на чистом клоне; 5) два прогона дают одинаковые SHA. Составь docs/DEMO.md: сценарий на 6 минут с точными командами и файлами, которые открываем, плюс 12 вероятных вопросов жюри с короткими ответами из PLAN §12.
```
